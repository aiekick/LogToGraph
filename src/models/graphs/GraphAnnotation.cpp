...
static inline double s_dot(ImPlotPoint a, ImPlotPoint b) {
    return a.x * b.x + a.y * b.y;
}
static inline ImPlotPoint operator-(ImPlotPoint v, ImPlotPoint f) {
    return ImPlotPoint(v.x - f.x, v.y - f.y);
}
static inline ImPlotPoint operator+(ImPlotPoint v, ImPlotPoint f) {
    return ImPlotPoint(v.x + f.x, v.y + f.y);
}
static inline ImPlotPoint operator*(double v, ImPlotPoint f) {
    return ImPlotPoint(v * f.x, v * f.y);
}
static inline ImPlotPoint operator*(ImPlotPoint f, double v) {
    return ImPlotPoint(v * f.x, v * f.y);
}
...
static inline double ImpLength(ImPlotPoint a) {
    return sqrt(ImpDot(a, a));
}
static inline double ImpDistance(ImPlotPoint A, ImPlotPoint B) {
    double dx = A.x - B.x;
    double dy = A.y - B.y;
...
bool GraphAnnotation::sIsMouseHoverLine2P(const ImVec2& vMousePos,
...
bool GraphAnnotation::sIsMouseHoverLine3P(const ImVec2& vMousePos,
                                          const double& vRadius,
                                          const ImVec2& vStart,
...
bool GraphAnnotation::sIsMouseHoverLine4P(const ImVec2& vMousePos,
                                          const double& vRadius,
                                          const ImVec2& vp0,
                                          const ImVec2& vp1,
                                          const ImVec2& vp2,
    ...
    if (m_ImGuiLabel) {
        ...
        if (win_ptr) {
            ...
            if (draw_list_ptr) {
                ...
                ImPlot::Annotation(m_LabelPos.x, m_LabelPos.y, m_Color, ImVec2(-15, 15), true, "%s", m_ImGuiLabel);
            }
        }
