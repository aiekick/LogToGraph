	...
	const SignalValue& vValue)
{
	...
	auto insert_query = ez::str::toStr(u8R"(insert or ignore into signal_ticks 
...
{
	auto insert_query = ez::str::toStr(u8R"(insert or ignore into signal_tags 
		...
		while (res == SQLITE_OK || res == SQLITE_ROW)
		{
			...
			if (res == SQLITE_OK || res == SQLITE_ROW)
			{
				...
				*/
				...
				{
					tag_color = ez::toImVec4(ez::fvariant(tag_color_string).GetV4(';') / 255.0f);
				}
