...
std::string LogEngine::sConvertEpochToDateTimeString(const double& vTime)
{
	...
	if (tm)
	{
		...
		return ez::str::toStr("%i/%i/%i %i:%i:%f",
	...
	if (!vName.empty())
	{
		...
		if (_datas_cat.find(vName) == _datas_cat.end()) // first value of the signal
		{
			...
			if (_datas_name_ptr)
			{
				...
				_datas_name_ptr->low_case_name_for_search = ez::toLower(vName); // save low case signal name for search
						...
						ptr && vHoveredTime <= ptr->time_epoch)
					{
						if (idx < (size_t)m_SignalsCount)
						{
							...
							if (ProjectFile::Instance()->m_AutoColorize)
							{
								...
								{
									parent_ptr->color_u32 = ImGui::GetColorU32(ez::toImVec4(GraphView::GetRainBow((int32_t)visible_idx, m_VisibleCount)));
			...
			if (strName == "signal")
			{
				...
				for (const tinyxml2::XMLAttribute* attr = vElem->FirstAttribute(); attr != nullptr; attr = attr->Next())
				{
					...
					if (attName == "category")
						...
						_signal_visibility = ez::ivariant(attValue).GetB();
					else if (attName == "color")
						_signal_color = ez::ivariant(attValue).GetU();
					else if (attName == "group")
						_signal_group = ez::ivariant(attValue).GetU();
				}
		...
		for (const auto& item_name : item_cat.second)
		{
			if (item_name.second.visibility) // par defaut c'est false, alors on sauve que ceux qui sont visibles
			{
				str += vOffset + "\t<signal category = \"" + item_cat.first + 
					...
					"\" color = \"" + ez::str::toStr(item_name.second.color) +
					"\" group = \"" + ez::str::toStr(item_name.second.group) + "\" />\n";
			}
		}
	}
