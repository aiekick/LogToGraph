...
void Messaging::DrawBar()
{
	...
	/*
	if (ImGui::MenuItem(ICON_FONT_REFRESH "##Refresh"))
	{
		LayoutManager::Instance()->ShowAndFocusSpecificPane(sMessagePaneId);
		Clear();
	}
	*/
	...
	if (!puMessages.empty())
	{
		...
		{
			if (ImGui::MenuItem(ICON_FONT_TRASH_O "##clear"))
			{
				Clear();
			}
		}
		...
		{
			if (ImGui::BeginMenu(ICON_FONT_TRASH_O "##clear"))
			{
	...
	if (!puMessages.empty())
	{
		if (puMessages.size() > 1)
		{
			...
			{
				LayoutManager::Instance()->ShowAndFocusSpecificPane(sMessagePaneId);
				currentMsgIdx = ez::maxi<int32_t>(--currentMsgIdx, 0);
			}
			if (ImGui::MenuItem(ICON_NDP2_CHEVRON_RIGHT_BOX "##right"))
			{
				LayoutManager::Instance()->ShowAndFocusSpecificPane(sMessagePaneId);
				currentMsgIdx = ez::maxi<int32_t>(++currentMsgIdx, (int32_t)puMessages.size() - 1);
			}
		...
		currentMsgIdx = ez::clamp<int32_t>(currentMsgIdx, 0, (int32_t)puMessages.size() - 1);
...
void Messaging::SortFields(SortingFieldEnum vSortingField, bool vCanChangeOrder)
{
	...
	if (vSortingField == SortingFieldEnum::FIELD_ID)
	{
		...
		{
			m_HeaderIdString = ICON_FONT_CHEVRON_DOWN + m_HeaderIdString;
		...
		{
			m_HeaderIdString = ICON_FONT_CHEVRON_UP + m_HeaderIdString;
	...
	else if (vSortingField == SortingFieldEnum::FIELD_TYPE)
	{
		...
		{
			m_HeaderTypeString = ICON_FONT_CHEVRON_DOWN + m_HeaderTypeString;
		}
		else
		{
			m_HeaderTypeString = ICON_FONT_CHEVRON_UP + m_HeaderTypeString;
		}
	}
	else if (vSortingField == SortingFieldEnum::FIELD_MSG)
	{
		...
		{
			m_HeaderMessageString = ICON_FONT_CHEVRON_DOWN + m_HeaderMessageString;
		}
		else
		{
			m_HeaderMessageString = ICON_FONT_CHEVRON_UP + m_HeaderMessageString;
		}
	}
...
{
	auto arr = ez::splitStringToVector(vMessage, '\n');
