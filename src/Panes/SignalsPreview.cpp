...
int SignalsPreview::DrawPanes(const uint32_t& /*vCurrentFrame*/, int vWidgetId, std::string /*vUserDatas*/, PaneFlag& vInOutPaneShown)
{
...
void SignalsPreview::DrawDialogsAndPopups(const uint32_t& /*vCurrentFrame*/, std::string /*vUserDatas*/)
{
	if (ProjectFile::Instance()->IsLoaded())
	{
...
int SignalsPreview::DrawWidgets(const uint32_t& /*vCurrentFrame*/, int vWidgetId, std::string /*vUserDatas*/)
{
	return vWidgetId;
}
						...
						ptr && vHoveredTime <= ptr->time_epoch)
					{
						if (idx < count_signals)
						{
							...
							if (ProjectFile::Instance()->m_AutoColorize)
							{
								...
								{
									parent_ptr->color_u32 = ImGui::GetColorU32(ct::toImVec4(GraphView::GetRainBow(visible_idx, visible_count)));
...
int SignalsPreview::DrawSignalButton(int& vWidgetPushId, SignalTickPtr vPtr, ImVec2 vGlyphSize)
{
	...
	if (vPtr)
	{
		...
		if (parent_ptr)
		{
			...
			ImVec4 color = ImLerp(ImVec4(0.1f, 0.1f, 0.1f, 1.0f), target_color, ratio);
			...
			if (signals_max_count_x)
			{
				...
				while (m_VirtualClipper.Step())
				{
					for (int j = m_VirtualClipper.DisplayStart; j < m_VirtualClipper.DisplayEnd; ++j)
					{
						...
						for (uint32_t i = 0; i < signals_max_count_x; ++i)
						{
